...
public class ProgramCodesAction extends ActionSupport implements Preparable, ServletRequestAware, ServletResponseAware {
     ...
     */
    private Map<Long, Integer> findStudyCancerCenterTargetAccrualsForFamily() {
        Map<Long, Integer> studyCCTargetAccruals = new HashMap<Long, Integer>();
        try {
            studyCCTargetAccruals = studyCancerCenterAccrualService
                    .getStudyCancerCenterAccrualsByFamily(selectedFamilyDTO.getPoId());
        } catch (Exception e) {
            LOG.error("Unable to find study cancer center target accruals", e);
        }
        return studyCCTargetAccruals;
    }
    ...
    private void populateStudyProtocolDTOs(JSONArray array) {
        ...
        Map<Long, Integer> studyCCTargetAccruals = findStudyCancerCenterTargetAccrualsForFamily();
        ...
        for (StudyProtocolQueryDTO dto : list) {
            ...
            tmpTrgtAcc = studyCCTargetAccruals.get(dto.getStudyProtocolId());
            object.put("targetAccrual", tmpTrgtAcc == null? "" : tmpTrgtAcc);
