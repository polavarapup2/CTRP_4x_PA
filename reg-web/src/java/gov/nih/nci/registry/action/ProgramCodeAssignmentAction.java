...
import gov.nih.nci.pa.service.util.StudyCancerCenterAccrualServiceRemote;
...
public class ProgramCodeAssignmentAction extends ActionSupport implements Preparable {
    ...
    private StudyCancerCenterAccrualServiceRemote studyCancerCenterAccrualService;
    ...
    public void prepare() {
        ...
        studyCancerCenterAccrualService = PaRegistry.getStudyCancerCenterAccrualService();
    }
    ...
    /**
     * @return the studyCancerCenterAccrualService
     */
    public StudyCancerCenterAccrualServiceRemote getStudyCancerCenterAccrualService() {
        return studyCancerCenterAccrualService;
    }

    /**
     * @param studyCancerCenterAccrualService the studyCancerCenterAccrualService to set
     */
    public void setStudyCancerCenterAccrualService(
            StudyCancerCenterAccrualServiceRemote studyCancerCenterAccrualService) {
        this.studyCancerCenterAccrualService = studyCancerCenterAccrualService;
    }
    ...
    /**
     * Will update study cancer center target accrual
     * and return success or error message
     *
     * @return json success or error in saving 
     * @throws IOException - when error
     */
    public StreamResult updateTargetAccrual() throws IOException {
        JSONObject root = new JSONObject();
        JSONArray arr = new JSONArray();
        root.put("data", arr);

        try {
            if (familyPoId != null && studyProtocolId != null && targetAccrual != null) {
                studyCancerCenterAccrualService.saveOrUpdateStudyCancerCenterAccrual(
                        familyPoId, studyProtocolId, targetAccrual);
            } else {
                throw new PAException("Required parameters missing");
            }

            return new StreamResult(new ByteArrayInputStream(root.toString().getBytes(UTF_8)));
        } catch (PAException pae) {
            LOG.error("error while updating target accrual", pae);
            root.put("success", false);
            root.put("msg", pae.getMessage());
            return new StreamResult(new ByteArrayInputStream(root.toString().getBytes(UTF_8))); 
        } catch (Exception e) {
            LOG.error("error while updating target accrual", e);
            root.put("success", false);
    ...
    private void populateTrials(Date startDate, Date endDate, JSONArray arr) throws PAException {
        ...
        Map<Long, Integer> studyCCTargetAccruals = studyCancerCenterAccrualService
                .getStudyCancerCenterAccrualsByFamily(familyPoId);
        Integer tmpTrgtAcc = null;
        ...
        for (StudyProtocolQueryDTO trial : trials) {
            ...
            tmpTrgtAcc = studyCCTargetAccruals.get(trial.getStudyProtocolId());
            o.put("targetAccrual", tmpTrgtAcc == null? "" : tmpTrgtAcc);
