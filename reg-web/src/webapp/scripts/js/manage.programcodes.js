...
function pgcinit($) {
    ...
    trailsTable = $('#trialsTbl').DataTable( {
        ...
        "columns": [
            {width:"12%", data: "identifiers", className:"scol"},
            {width: "22%", data:function(row, type, val, meta){
            ...
            {data: "piFullName" , className:'scol'},
            {data: "trialStatus",  className: 'scol'},
            {width: "12%",data: function(row, type, val, meta){
                return "<a href='#' id='" + row.studyProtocolId + "_tgtacc' class='trltgtacceditable' data-pk='"+ row.studyProtocolId +"'>" + row.targetAccrual + "</a>" ;
            }},
            {width:"12%",
                orderable: false ,
                className:"pgctd",
        ...
        "fnDrawCallback": function( nRow, aData, iDisplayIndex, iDisplayIndexFull ) {
            $('#trialsTbl a.trltgtacceditable').editable({
                type: 'text',
                params: function(params) {
                	params.familyPoId = $("#familyPoId").val();
                	params.studyProtocolId = params.pk;
                	params.targetAccrual = params.value;
                	return params;
                },
                url: 'managePCAssignmentupdateTargetAccrual.action',
    ...
    $('#trialsTbl tbody').on('click', 'tr', function (evt) {
       // evt.preventDefault(); //commented out for PO-10005
