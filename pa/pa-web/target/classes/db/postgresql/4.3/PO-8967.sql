delete from PA_PROPERTIES where name='dashboard.workload.milestones';
insert into PA_PROPERTIES values ((select max(identifier) + 1 from pa_properties),
    'dashboard.workload.milestones','Submission Received Date,Submission Acceptance Date,Submission Reactivated Date,Administrative Processing Start Date,Administrative Processing Completed Date,Ready for Administrative QC Date,Administrative QC Start Date,Administrative QC Completed Date,Scientific Processing Start Date,Scientific Processing Completed Date,Ready for Scientific QC Date,Scientific QC Start Date,Scientific QC Completed Date,Ready for Trial Summary Report Date');
    
drop VIEW rv_search_results;

CREATE OR REPLACE VIEW rv_last_hold AS
    SELECT study_protocol_identifier, onhold_reason_code, onhold_date, offhold_date, onhold_reason_text, onhold_reason_category   
