CREATE OR REPLACE VIEW rv_biomarker_existence AS
	select sp.identifier as study_protocol_identifier, true as planned_marker_existence_indicator, min(pm.identifier) as first_marker_id from PLANNED_MARKER pm 
	    inner join PLANNED_ACTIVITY pa on pm.IDENTIFIER=pa.IDENTIFIER 
	    inner join STUDY_PROTOCOL sp on pa.STUDY_PROTOCOL_IDENTIFIER=sp.identifier
...
CREATE OR REPLACE VIEW rv_search_results AS 
 SELECT sp.identifier AS study_protocol_identifier, 
        ...
        biomarker_existence.planned_marker_existence_indicator
   ...
   LEFT JOIN rv_biomarker_existence biomarker_existence ON sp.identifier = biomarker_existence.study_protocol_identifier;