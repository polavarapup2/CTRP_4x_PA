...
CREATE OR REPLACE VIEW rv_dcp_id AS
...
WHERE functional_code = 'IDENTIFIER_ASSIGNER' and org.name='National Cancer Institute Division of Cancer Prevention';

CREATE OR REPLACE VIEW rv_ctep_id AS
SELECT ss.study_protocol_identifier, ss.local_sp_indentifier
FROM study_site ss
INNER JOIN research_organization ro ON (ro.identifier = ss.research_organization_identifier)
INNER JOIN organization org ON (org.identifier = ro.organization_identifier)
WHERE functional_code = 'IDENTIFIER_ASSIGNER' and org.name='Cancer Therapy Evaluation Program';
