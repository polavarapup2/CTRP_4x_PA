CREATE OR REPLACE VIEW rv_sponsor_organization AS
...
WHERE functional_code = 'SPONSOR';

CREATE OR REPLACE VIEW rv_organization_responsible_party AS
SELECT ss.study_protocol_identifier, org.assigned_identifier, org.name, ss.local_sp_indentifier
FROM study_site ss
JOIN research_organization ro ON (ro.identifier = ss.research_organization_identifier)
JOIN organization org ON (org.identifier = ro.organization_identifier)
WHERE functional_code = 'RESPONSIBLE_PARTY_SPONSOR';
...
SELECT sp.identifier                    study_protocol_identifier
      ...
      ,sp.phase_code                    phase_code
      ,sp.primary_purpose_code          primary_purpose_code
      ,sp.start_date                    start_date 
      ,sr.type_code                     summary4fundingSponsor_type
      ,so.name                          sponsor_name
      ,orp.name                         responsible_party_organization_name
      ,pirp.first_name                  responsible_party_PI_first_name
      ,pirp.last_name                   responsible_party_PI_last_name
      ,upd_usr.login_name               user_last_updated_login
      ,upd_usr.first_name               user_last_updated_first
      ,upd_usr.last_name                user_last_updated_last
...
LEFT OUTER JOIN study_resourcing sr ON (sp.identifier = sr.study_protocol_identifier)
LEFT OUTER JOIN rv_sponsor_organization so ON (sp.identifier = so.study_protocol_identifier)
LEFT OUTER JOIN rv_organization_responsible_party orp ON (sp.identifier = orp.study_protocol_identifier)
LEFT OUTER JOIN rv_principal_investigator_responsible_party pirp ON (sp.identifier = pirp.study_protocol_identifier)
LEFT OUTER JOIN csm_user upd_usr ON (sp.user_last_updated_id = usr.user_id);
