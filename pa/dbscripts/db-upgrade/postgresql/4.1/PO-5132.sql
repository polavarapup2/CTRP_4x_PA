ALTER TABLE study_protocol ADD COLUMN submitting_organization_id bigint;

ALTER TABLE study_protocol
  ADD CONSTRAINT "fk_study_protocol_submiting_organization -> organization" FOREIGN KEY (submitting_organization_id)
      REFERENCES organization (identifier) MATCH SIMPLE
      ON UPDATE NO ACTION ON DELETE NO ACTION;
      
UPDATE study_protocol SET submitting_organization_id = (SELECT org.identifier FROM organization org 
	INNER JOIN registry_user ru ON org.assigned_identifier = CAST(ru.affiliated_org_id AS VARCHAR)
	WHERE ru.csm_user_id = study_protocol.user_last_created_id);

CREATE OR REPLACE VIEW rv_trial_submitter AS 
 SELECT sp.identifier AS study_protocol_identifier, 
    ru.identifier AS submitter_id, ru.first_name AS submitter_fname, 
    ru.last_name AS submitter_lname, o.name AS submitter_org_name, 
    o.assigned_identifier AS submitter_org_id
   FROM registry_user ru
   JOIN csm_user cu ON cu.user_id = ru.csm_user_id
